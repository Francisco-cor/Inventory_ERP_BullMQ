...
export async function runRetention(): Promise<number> {
  ...
  const { rowCount: c4 } = await pool.query(
    `DELETE FROM movimientos_stock WHERE creado_en < NOW() - INTERVAL '1 day' * $1`,
    [RETENTION_DAYS]
  );
  const total = (c1 ?? 0) + (c2 ?? 0) + (c3 ?? 0) + (c4 ?? 0);
  if (total > 0) console.log(`[retention:stock] deleted ${total} rows`);
  return total;
}
...
export function startRetentionJob(): void {
  ...
  console.log(`[retention:stock] scheduled`);
}

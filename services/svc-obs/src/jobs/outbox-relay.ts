...
export function startOutboxRelay(): void {
  if (timer) return;
  console.log(`[outbox-relay:obs] started — poll ${POLL_INTERVAL_MS}ms`);
  timer = setInterval(() => void tick().catch((e) => console.error("[outbox-relay:obs] tick failed", e)), POLL_INTERVAL_MS);
  void tick().catch(() => {});
}
  ...
  try {
    ...
    for (const row of rows) {
      ...
      try {
        ...
        console.log(JSON.stringify({ level: "info", service: "svc-obs", outboxId: row.id, eventName: row.nombre_evento, msg: "outbox published" }));
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        await client.query(`UPDATE outbox SET attempts = attempts + 1, last_error = $2 WHERE id = $1`, [row.id, msg.slice(0, 1000)]);
        console.error(JSON.stringify({ level: "error", service: "svc-obs", outboxId: row.id, error: msg, msg: "outbox publish failed" }));
      }
    }
    await client.query("COMMIT");
  } catch (err) {
